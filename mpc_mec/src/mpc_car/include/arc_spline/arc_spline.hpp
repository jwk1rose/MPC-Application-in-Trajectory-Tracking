...
double spline::operator()(double x, int dd) const
{
  ...
  // {
  // std::cout << "x=" << x << "m_x.front=" << m_x.front() << "m_x.back()=" << m_x.back() << "m_x.size=" << m_x.size()
  // << std::endl;
  assert(x >= m_x.front() && x <= m_x.back());
  // }
  // std::cout << "x=" << x << "m_x.front=" << m_x.front() << "m_x.back()=" << m_x.back() << std::endl;
  // assert(x >= m_x.front() && x <= m_x.back());
...
public:
  ArcSpline(){};
  // ArcSpline(const ArcSpline&){};

  ~ArcSpline(){};
  void reset();
  void setWayPoints(const std::vector<double>& x_waypoints, const std::vector<double>& y_waypoints);
...
void ArcSpline::setWayPoints(const std::vector<double>& x_waypoints, const std::vector<double>& y_waypoints)
{
  ...
  {
    double left_arc = res * std::sqrt(xt_(t - res, 1) * xt_(t - res, 1) + yt_(t - res, 1) * yt_(t - res, 1));
  ...
  // std::cout << "xL_.size()= " << xL_.size() << "yL_.size()= " << yL_.size() << std::endl;
